...
class ReadBMPFile:
    ...
    def BGR2YCbCr(self):
        """把图片从RBG转换为YCrCb"""
        B = np.array(self.B, dtype=np.uint8)
        G = np.array(self.G, dtype=np.uint8)
        R = np.array(self.R, dtype=np.uint8)
        self.Y = np.around(0.299*R + 0.587*G + 0.114*B).astype(int)
        self.Cb = np.around(-0.1687*R - 0.3313*G + 0.5*B + 128).astype(int)
        self.Cr = np.around(0.5*R - 0.4187*G - 0.0813*B + 128).astype(int)


if __name__ == "__main__":
    obj = ReadBMPFile('lena512color.bmp')
